...
function encodeMessage() {
  ...
  var text = $('textarea.message').val().trim();
...
function decodeMessage() {
  ...
  for (k = 0; k < output.length; k++) {
    if (regexp.test(output.charAt(k)) || /\s/g.test(output.charAt(k))) {
      finaloutput += output.charAt(k);
    }
  }
  $('.binary-decode textarea').text(finaloutput);
